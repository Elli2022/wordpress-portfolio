...
add_filter('graphql_request_data', function ($data, $request_context) {
    ...
    if (isset($data['query']) && is_string($data['query'])) {
        // Keep the response key as `PostInfo` while resolving with compat field.
        $data['query'] = preg_replace(
            '/\bPostInfo\b(?!\s*:)/',
            'PostInfo: postInfoCompat',
            $data['query']
        );

        // Compat for WPGraphQL ACF image shape differences.
        $data['query'] = preg_replace(
            '/projectImage\s*\{\s*mediaItemUrl\s*\}/',
            'projectImage { node { mediaItemUrl } }',
            $data['query']
        );
    }
